...
Notice the format the contrast-based data takes. The object should have one row per arm (the same as arm-based data), but the reference arm has `NA`'s in the difference (here, called `y`) and standard error (here, called `se.diffs`) columns. If the network includes multi-arm trials, we require an additional column used to specify the variance of the response in the reference arm for each trial, which has a variance in the rows corresponding to a reference arm and and `NA`s elsewhere.
...
contrast_prep <- data.prep(arm.data = contrast,
                           varname.t = "Treatment",
                           varname.s = "Study")
```
...
The network has 4 treatments and is not highly connected.

## Main Analysis

When contrast-based summary data is used, the Normal distribution is used with the identity link for all types of contrast data. Some examples of contrast-based data summaries are log odds ratios, mean differences, log hazard ratios, and risk differences. A mathematical description of the model follows.
...
The DIC is slightly less for the fixed effect model. The leverage plots are similar between the two models. Overall, we will continue with the fixed effects model for demonstration purposes.
...
Here, the consistency and inconsistency models seem almost equivalent. 
...
The data lies very near the line $y=x$, indicating agreement between the models. This suggests we can proceed with the more parsimonious model, the consistency model.
...
```{r}
nma.diag(fixed_effects_results)

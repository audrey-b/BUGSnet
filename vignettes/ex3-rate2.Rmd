...
```{r}
random_effects_model$bugs
```

###Assessment of convergence of final model

After a model is fit to the evidence, check the trace plots of the model using `nma.trace()`. Look for spikes or general irregularities in the posterior distributions of the $d$'s. Irregularities in these distributions suggest that the model never converged, and will likely require you to run more have a longer burn-in period, and more iterations. If you wish to know which treatments are causing irregularities, you can use the command `random_effects_model$trt.key` to see which treatment correspond to which integer number on the traceplot. Here, the model shows strong signs of convergence.

...
Before any analysis is done, we need to put our data in a form that BUGSnet can easily handle. This is done by simple running the `data.slr()` function and specifying the name of the treatment, study, and sample size variables:
``` {r}
...
The following BUGS code is produced by the `nma.analysis()` function.
```{r}

...
### SUCRA Plot
SUCRA plots provide a simulaneous comparison of every treatment based on the results of the NMA analysis. Each treatment is assigned a probability of being the best, second best,...,worst rank. In BUGSnet, simply input the results of your model into the `nma.rank()` function, and specify `largerbetter=TRUE` if a larger outcome is associated with better treatments, and  `FALSE` otherwise.
...
From this plot, we can see that there is a $\sim$ 77% probability that ARB's are the best treatments, and a $\sim$ 80% probability that diuretics are the worst treatment. Since this plot was produced using ggplot2, one can easily customize the plot using ggplot2 commands, for instance:

```{r, echo=FALSE, fig.width=7, fig.height=4, dpi = 95, fig.align="centre"}
sucra.out$sucra + theme_gray()
```

The same data can be displayed in the form of a rankogram.

```{r, echo=TRUE, fig.width=7, fig.height=4, dpi = 95}
sucra.out$rankogram
```

If you want the exact percentages, these results are also available in tabular form via `sucra.out$table`.
```{r, echo = FALSE}
kable(sucra.out$table)
```
